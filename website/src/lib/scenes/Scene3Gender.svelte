...
<section class="scene-gender" aria-labelledby="scene-3-title">
  <div class="scene-grid">
    <div class="sticky-chart">
      ...
      {/if}
      <p class="source">
        Source: MoMA Collection. n = {lineData.reduce((sum, d) => sum + d.n, 0).toLocaleString()} credited works.
      </p>
    </div>
    <div class="text-rail">
      <ScrollStep scene={3} step={0}
        ><p>The first decades are noisy: a few credited works can swing the share sharply.</p></ScrollStep
      >
      <ScrollStep scene={3} step={1}
        ><p>
          The durable change begins much later. The curve rises after the 1980s and approaches, but does not reach,
          parity.
        </p></ScrollStep
      >
      <ScrollStep scene={3} step={2}
        ><p>
          Departments move at different speeds. Small multiples keep the same scale so the contrast is not exaggerated.
        </p></ScrollStep
      >
    </div>
  </div>
